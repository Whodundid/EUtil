...
public class ExpandableGrid<E> {
    ...
    private static <E> EList<E> newRow(int width, E defaultValue) {
        EList<E> r = new EArrayList<>(width);
    ...
    public EList<E> getRow(int rowNum) {
        return grid.get(rowNum);
    }
    
    /** Gets an entire column of values in this Grid. */
    public EList<E> getCol(int colNum) {
        EList<E> col = new EArrayList<>(height);
        for (int i = 0; i < height; i++) {
    ...
    public void setSize(int widthIn, int heightIn, E defaultValue) {
        width = widthIn;
        height = heightIn;
        grid = new EArrayList<>(height);
        for (int i = 0; i < height; i++) {
    ...
    public void setCol(List<E> in, int colNum) {
        ...
        for (int i = 0; i < height; i++) {
            grid.get(i).set(colNum, in.get(i));
        }
    }
